...
// QueryPartnerAddress chains the current query on the "partner_address" edge.
func (_q *PartnerQuery) QueryPartnerAddress() *EventQuery {
...
func (_q *PartnerQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *PartnerQuery) FirstIDX(ctx context.Context) int {
...
func (_q *PartnerQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *PartnerQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *PartnerQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *PartnerQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *PartnerQuery) Clone() *PartnerQuery {
	...
	return &PartnerQuery{
		...
		withPartnerAddress: _q.withPartnerAddress.Clone(),
...
// WithPartnerAddress tells the query-builder to eager-load the nodes that are connected to
// the "partner_address" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PartnerQuery) WithPartnerAddress(opts ...func(*EventQuery)) *PartnerQuery {
	query := (&EventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPartnerAddress = query
	return _q
}
...
func (_q *PartnerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Partner, error) {
	...
	if query := _q.withPartnerAddress; query != nil {
		if err := _q.loadPartnerAddress(ctx, query, nodes,
			func(n *Partner) { n.Edges.PartnerAddress = []*Event{} },
			func(n *Partner, e *Event) { n.Edges.PartnerAddress = append(n.Edges.PartnerAddress, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PartnerQuery) loadPartnerAddress(ctx context.Context, query *EventQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *Event)) error {
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(partner.PartnerAddressColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.partner_partner_address
		if fk == nil {
			return fmt.Errorf(`foreign-key "partner_partner_address" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "partner_partner_address" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *PartnerQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(partner.Table, partner.Columns, sqlgraph.NewFieldSpec(partner.FieldID, field.TypeInt))
	_spec.From = _q.sql
