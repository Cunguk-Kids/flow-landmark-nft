...
func (_q *LikeQuery) QueryMoment() *NFTMomentQuery {
...
func (_q *LikeQuery) WithMoment(opts ...func(*NFTMomentQuery)) *LikeQuery {
	query := (&NFTMomentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LikeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Like, error) {
	...
	if query := _q.withMoment; query != nil {
		if err := _q.loadMoment(ctx, query, nodes, nil,
			func(n *Like, e *NFTMoment) { n.Edges.Moment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LikeQuery) loadMoment(ctx context.Context, query *NFTMomentQuery, nodes []*Like, init func(*Like), assign func(*Like, *NFTMoment)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Like)
	for i := range nodes {
		if nodes[i].nft_moment_likes == nil {
			continue
		}
		fk := *nodes[i].nft_moment_likes
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "nft_moment_likes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
