...
// was not loaded in eager-loading, or loaded but was not found.
func (e CommentEdges) MomentOrErr() (*NFTMoment, error) {
	if e.Moment != nil {
		return e.Moment, nil
...
func (*Comment) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case comment.ForeignKeys[0]: // nft_moment_comments
			values[i] = new(sql.NullInt64)
	...
	for i := range columns {
		...
		case comment.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field nft_moment_comments", value)
			} else if value.Valid {
				_m.nft_moment_comments = new(int)
				*_m.nft_moment_comments = int(value.Int64)
			}
...
// QueryMoment queries the "moment" edge of the Comment entity.
func (_m *Comment) QueryMoment() *NFTMomentQuery {
	return NewCommentClient(_m.config).QueryMoment(_m)
}
