...
// was not loaded in eager-loading, or loaded but was not found.
func (e LikeEdges) MomentOrErr() (*NFTMoment, error) {
	if e.Moment != nil {
		return e.Moment, nil
...
func (*Like) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case like.ForeignKeys[0]: // nft_moment_likes
			values[i] = new(sql.NullInt64)
	...
	for i := range columns {
		...
		case like.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field nft_moment_likes", value)
			} else if value.Valid {
				_m.nft_moment_likes = new(int)
				*_m.nft_moment_likes = int(value.Int64)
			}
...
// QueryMoment queries the "moment" edge of the Like entity.
func (_m *Like) QueryMoment() *NFTMomentQuery {
	return NewLikeClient(_m.config).QueryMoment(_m)
}
