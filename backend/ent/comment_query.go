...
func (_q *CommentQuery) QueryMoment() *NFTMomentQuery {
...
func (_q *CommentQuery) WithMoment(opts ...func(*NFTMomentQuery)) *CommentQuery {
	query := (&NFTMomentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if _q.withUser != nil || _q.withMoment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withMoment; query != nil {
		if err := _q.loadMoment(ctx, query, nodes, nil,
			func(n *Comment, e *NFTMoment) { n.Edges.Moment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CommentQuery) loadMoment(ctx context.Context, query *NFTMomentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *NFTMoment)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].nft_moment_comments == nil {
			continue
		}
		fk := *nodes[i].nft_moment_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(nftmoment.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "nft_moment_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
