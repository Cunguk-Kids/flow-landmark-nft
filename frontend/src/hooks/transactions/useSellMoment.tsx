...
transaction(
    ...
    let accessoryCollection: &NFTMoment.Collection
    ...
    prepare(acct: auth(BorrowValue, IssueStorageCapabilityController, PublishCapability, SaveValue, StorageCapabilities) &Account) {
        ...
        self.accessoryCollection = acct.storage.borrow<&NFTMoment.Collection>(from: collectionData.storagePath)
...
export function useSellMoment() {
  ...
  const sell = (nftId: number, price: number) => {
    ...
    mutate({
      cadence: SELL_ACCESSORY_TX,
      args: (arg, t) => [
        arg(String(nftId), t.UInt64),        // saleItemID
        arg(priceStr, t.UFix64),             // saleItemPrice
        arg("harkon-market-v1", t.Optional(t.String)), // customID
        arg("0.0", t.UFix64),                // commissionAmount (0 dulu)
        arg(String(expiry), t.UInt64),       // expiry
        arg([], t.Array(t.Address)),         // marketplacesAddress (kosong)
        arg(NFT_TYPE_ID, t.String),          // nftTypeIdentifier
        arg(FT_TYPE_ID, t.String)            // ftTypeIdentifier
      ]
    });
  };

  return { sell, isPending, isSealed, error, txId };
}