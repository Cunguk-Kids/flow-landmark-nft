...
function EventDetailPage() {
  ...
  const { data: event, isLoading } = useEventDetail(eventId, user?.addr);
  ...
  const handleCheckIn = () => {
    // Panggil fungsi mutasi dengan ID event
    checkIn(eventId); 
  };
  ...
  useEffect(() => {
    if (isRegistered) {
      ...
      // refetchEventDetail(); // Jika perlu refresh kuota
    }
  }, [isRegistered]);
            ...
            <div className="relative">
                <div className="sticky top-24">
                    <div className="bg-rpn-card border-2 border-rpn-blue rounded-2xl p-6 shadow-[8px_8px_0px_0px_rgba(41,171,226,0.2)]">
                        ...
                        {/* TOMBOL AKSI */}
                        {/* Di sini Anda akan memasukkan logika useCheckIn / useRegister */}
                        {registerError && <p className="text-red-500 text-xs mb-2">Gagal: {registerError.message}</p>}

                        <Button 
                            onClick={() => register(eventId)}
                            // Disable jika: Sedang loading ATAU Belum Login ATAU SUDAH REGISTER
                            ...
                            className={`
                                ...
                                ${event.isRegistered 
                                    ? "bg-green-900/50 text-green-500 border-2 border-green-900 cursor-default hover:bg-green-900/50 shadow-none" // Style Pasif (Registered)
                                    : "bg-rpn-blue hover:bg-white hover:text-rpn-blue text-rpn-dark" // Style Aktif (Register)
                                }
                            `}
                        >
                            {/* Logika Teks Tombol */}
                        ...
                        {isError && <p className="text-red-500 text-xs mb-2">user already check in</p>}
                        <Button 
                            onClick={handleCheckIn}
                            disabled={isCheckingIn}
                            className="w-full h-14 font-black text-lg uppercase rounded-xl shadow-lg transition-all bg-rpn-blue hover:bg-white hover:text-rpn-blue text-white"
                        >
                            {isCheckingIn ? "Checkin..."  : "Checkin Now"}
                        </Button>
