...
import { User, Heart, MessageCircle, Share2, MapPin } from "lucide-react";
...
export default function MomentPostCard({ moment }: MomentPostCardProps) {
  ...
  const isLiked = moment.is_liked; // Assumes backend returns this
  const likeCount = moment.like_count || 0; // Assumes backend returns this (optional for now)
    ...
    <div className="bg-white border-2 border-rpn-dark rounded-xl overflow-hidden shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] mb-8">
      ...
      <div className="p-4 flex items-center justify-between border-b-2 border-rpn-dark/10 bg-gray-50/50">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-rpn-blue border-2 border-black overflow-hidden">
            {owner.pfp ? (
              <img
                src={owner.pfp}
                alt={owner.nickname}
          ...
          <div>
            <h3 className="font-bold text-rpn-dark text-sm leading-none">
