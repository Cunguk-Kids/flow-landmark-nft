...
access(all) contract NFTMoment: NonFungibleToken {
    ...
    access(all) resource NFT: NonFungibleToken.NFT {
        ...
        access(all) var mergedFrom: [UInt64] // IDs of NFTs that were merged into this one
    ...
    access(all) resource Collection: NonFungibleToken.Collection {
        ...
        access(all) fun borrowNFTMoment(id: UInt64): &NFTMoment.NFT? {
            if self.ownedNFTs[id] != nil {
                let ref = &self.ownedNFTs[id] as &{NonFungibleToken.NFT}?
                return ref as! &NFTMoment.NFT
            }
            return nil
        }
        ...
        access(all) fun deposit(token: @{NonFungibleToken.NFT}) {
            let nft <- token as! @NFTMoment.NFT
            let id = nft.id
        ...
        access(all) fun getNFTsByCategory(category: Category): [UInt64] {
            ...
            for id in self.ownedNFTs.keys {
                let nft = self.borrowNFTMoment(id: id)
    ...
    access(all) fun upgradeNFT(collection: &Collection, id: UInt64, newRarity: Rarity) {
        let nft = collection.borrowNFTMoment(id: id) ?? panic("NFT not found")
    ...
    access(all) fun mergeNFTs(
        ...
        // Withdraw both NFTs
        let nft1 <- collection.withdraw(withdrawID: id1) as! @NFTMoment.NFT
        let nft2 <- collection.withdraw(withdrawID: id2) as! @NFTMoment.NFT
